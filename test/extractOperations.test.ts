...
describe('extractOperations', () => {
	...
	it('extracts requestBody schema', async () => {
		...
		const properties = createPet?.requestBody?.schema?.properties as Record<string, unknown>;
	...
	it('extracts dereferenced requestBody schema', async () => {
		...
		expect(updatePet?.requestBody).toBeDefined();
		expect(updatePet?.requestBody?.contentType).toBe('application/json');
		expect(updatePet?.requestBody?.schema?.type).toBe('object');
		const properties = updatePet?.requestBody?.schema?.properties as Record<string, unknown>;
		expect(properties?.name).toBeDefined();
	});
	...
	it('handles operations without requestBody', async () => {
		...
		const getPet = operations.find((op) => op.operationId === 'getPet');
		expect(getPet?.requestBody).toBeUndefined();
	});
});
